...
# ╔═╡ 46ef976f-5eba-4ccf-89a6-7fab8e1a6ad4
begin
	data =X["X"]
	x₁ = data[1, :]
	x₂ = data[2, :]
...
# ╔═╡ f5b2d3ce-5790-4dc7-81a0-a722f83d278d
function viz_lake(x₁::Vector, x₂::Vector)
	fig = Figure()
	ax = Axis(fig[1,1], xlabel = "x₁ [miles]", ylabel = "x₂ [miles]", 
		       aspect= DataAspect(), title ="Carter Lake Boundary")
	scatter!(x₁, x₂, color = :blue)
	fig
end

# ╔═╡ d06b5983-3514-4c36-815c-5c53397cc244
viz_lake(x₁, x₂)
...
r = [sqrt(xᵢ^2 + xᵢᵢ^2) for (xᵢ, xᵢᵢ) in zip(x̂₁, x̂₂)]
...
θs = [atan(xᵢᵢ, xᵢ) for (xᵢᵢ, xᵢ) in zip(x̂₂, x̂₁)]

# ╔═╡ ba93499d-3e10-43b1-91f5-41078459f50a
begin
	x′₁ = [rᵢ * cos(θ) for (rᵢ, θ) in zip(r, θs)]
	x′₂ = [rᵢ * sin(θ) for (rᵢ, θ) in zip(r, θs)]
end
...
interp_linear = linear_interpolation(θs_sorted, r_sorted)
...
# ╔═╡ 32a642e5-9b39-4f92-a19d-f2e6420ed9f9
begin
	insert!(θs_sorted, 1, θs_sorted[end]- 2*π)
	insert!(r_sorted, 1, r_sorted[end])
end

# ╔═╡ 2e3bd54d-874c-4e19-b527-5e117017ed0e
θs_sorted
...
periodic_interpolation = linear_interpolation(θs_sorted, r_sorted, extrapolation_bc=Periodic())
...
begin
	θ = collect(range(-pi, pi, length=500))
	rs= periodic_interpolation(θ)
end

# ╔═╡ dd47e2ed-45b4-4693-afb3-7f03c6c3aa2e
function plot_continous_lake(r::Vector, θ::Vector)
	xᵢ = [rᵢ * cos(θ) for (rᵢ, θ) in zip(r, θ)]
	xᵢᵢ = [rᵢ * sin(θ) for (rᵢ, θ) in zip(r, θ)]
	
	fig = Figure()
	ax = Axis(fig[1,1], xlabel = "x₁ [miles]", ylabel = "x₂ [miles]", 
		       aspect= DataAspect(), title ="Carter Lake Boundary")
	lines!(xᵢ, xᵢᵢ, color = :blue)
	fig
	
end

# ╔═╡ db3bf747-2ac9-47af-b095-c0ca58467917
plot_continous_lake(rs, θ)
...
function inside_lake(x::Vector, r_of_θ = periodic_interpolation)
	x₁, x₂ = x
	θ̃ = atan(x₂, x₁)
	r̃ = sqrt(x₂^2 + x₁^2)
	r = r_of_θ(θ̃)
...
# ╔═╡ be06482a-3a0e-4b76-ba7d-b404019d90c7
inside_lake([-2.0, 2.0])
...
function run_monte_carlo(N::Int64, L::Float64, r_of_θ = periodic_interpolation)
	dist = Uniform(-L/2, L/2)
	X=rand(dist, (2,N))
	x₁ =X[1, :]
	x₂ = X[2, :]
	inside_lake_check = zeros(N)
	for i in 1:N
		θ̃ = atan(x₂[i], x₁[i])
		r̃ = sqrt(x₂[i]^2 + x₁[i]^2)
		r = r_of_θ(θ̃)
		if r̃>r
			inside_lake_check[i] = false
		else
			inside_lake_check[i] = true
		end
	end
	return (location=X, check=inside_lake_check)
end
...
# ╔═╡ 9fa92263-19e0-4a7b-9211-dfc329e719c1
begin
	N = 10000
	monte_carlo = run_monte_carlo(N, L)
end

# ╔═╡ 09076209-2dba-4314-b138-f209dbd68d3f
area_of_lake = L^2 * sum(monte_carlo.check) /N
...
function draw_monte_carlo()
	fig =Figure()
	ax = Axis(fig[1,1], xlabel = "x₁ [miles]", ylabel = "x₂ [miles]", 
		       aspect= DataAspect(), title = "Monte Carlo Lake Viz")
	xᵢ = [rᵢ * cos(θ) for (rᵢ, θ) in zip(rs, θ)]
	xᵢᵢ = [rᵢ * sin(θ) for (rᵢ, θ) in zip(rs, θ)]
	lines!(xᵢ, xᵢᵢ, color=:blue)

	x₁ =monte_carlo.location[1, :]
	x₂ = monte_carlo.location[2, :]
	
	for (i,x̂₁) in enumerate(x₁)
		θ̃ = atan(x₂[i], x̂₁)
		r̃ = sqrt(x₂[i]^2 + x̂₁^2)
		r = periodic_interpolation(θ̃)
		if r̃>r
			scatter!(ax, x̂₁, x₂[i], marker=:+, color=:brown)
		else
			scatter!(ax, x̂₁, x₂[i], color=:blue)
		end
	end
	fig
end

# ╔═╡ c0aaccfa-e1db-40d6-bb3a-3edaa5c0d63f
draw_monte_carlo()
...
# ╔═╡ ac4559e4-9636-4a9a-94fe-f97bb7facc62
md"""
## Answer
 * The difference between the variance is due to the number of times we sampled. When the "raindrop" is small, the points might all be inside the lake area, outside the square, and vice versa. But with a larger amount of raindrops, we will have enough samples to describe the area of the lake address  

* I will probably get the polar coordination of the island, then plot it in the graph such that the island's area is inside the lake. Then, using a for loop, I love for drops that fall only in the lake. 

* I will say we should probably split up the area of the lake and model them diferently, then sum the individual segments together to get the total area. I think this can be done by cuts along the lake such that it touches land on both ends. That will give a shape of an arc on a cord and we can parameterized that as $r=r(\theta)$. Then the remaining area of the lake that was cut off by the line can be modelled diferently and the sum of all the segment should be the area of the lake
"""
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "64d4529823a28e4493f249dff074ea3706278bdc"
